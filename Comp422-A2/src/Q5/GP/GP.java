...
public class GP
extends GPProblem {
	private transient static Logger LOGGER = Logger.getLogger(GP.class);
	...
	public GP(GPConfiguration a_conf)
	...
	public GPGenotype create()
		...
		if (useADF) {
			nodeSets = new CommandGene[2][numInputVariables +  command_len];
		}
		else{
			nodeSets = new CommandGene[1][numInputVariables +  command_len];
		}
		// the variables:
	...
	public void main(String[] args)
		...
		//
		String outputFileName = "GPOutputFeature";
		if (args.length > 0) {
		...
		config.setFitnessFunction(new GP.FormulaFitnessFunction());
		/**
		...
		testFittest(fittest,outputFileName);

		config.reset();

	}

	public static void testFittest(IGPProgram ind,String outputFileName) {
		...
		for (int j = 0; j < numRows; j++) {
			...
			try {
				...
				double expected = data[outputVariable][j];

				pw.println(String.format("%9.9f , %1.0f", result,expected));
		...
		try {
			pw = new PrintWriter(new File("tempFunction"));

			pw.println(ind.toStringNorm(0));
	...
	extends GPFitnessFunction {
		...
		public double computeRawFitness(final IGPProgram ind) {
			...
			List<String> actualClass1 = new ArrayList<String>();
			List<String> actualClass2 = new ArrayList<String>();
			List<String> actualClass3 = new ArrayList<String>();

			List<Double> class2Values = new ArrayList<Double>();
			...
			for (int j = 0; j < numRows; j++) {
				...
				try {
					...
					if(data[outputVariable][j] == 2){
						class2Values.add(result);
					}
			...
			Collections.sort(class2Values);

			class2Min = class2Values.get(0);
			class2Max = class2Values.get(class2Values.size() - 1 );
...
/*					if((int)result + 0.5 != data[outputVariable][j]){
						error += 1 * Math.pow(Math.abs(result - data[outputVariable][j]),2);
					}*/

					//results.add(Math.abs(result - data[outputVariable][j]));

					double difference = ((result - data[outputVariable][j]));

					if(data[outputVariable][j] == 1){
						actualClass1.add("1");
						actualClass1.add(getExpected(result,class2Min,class2Max));
					}

					if(data[outputVariable][j] == 2){
						actualClass2.add("2");
						actualClass2.add(getExpected(result,class2Min,class2Max));

					}

					if(data[outputVariable][j] == 3){
						actualClass3.add("3");
						actualClass3.add(getExpected(result,class2Min,class2Max));
			...
			//error = error * results.get(results.size() - 1);
			error = calculateShannonEntropy(actualClass1);
			error += calculateShannonEntropy(actualClass2);
			error += calculateShannonEntropy(actualClass3);
			...
			//return Math.sqrt((error/numRows));

		}

	}

	public static String getExpected(double value, double class2Min, double class2Max){
		if(value < class2Min){
			return "1";
		}
		else if(value > class2Min
				&& value <= class2Max){
			return "2";
		}
		else{
